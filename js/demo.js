// Get a reference to your clickable element
var hell = document.getElementById('hell');

// Add a click event listener to your element
hell.addEventListener('click', function () {
    // Get a reference to the iframe
    var myIframe = document.getElementById('bio');

    // Set the src attribute of the iframe to load your content
    myIframe.src = 'Biography/index.html';
});
/**
 * demo.js
 ...
 */
{
    ...
    class Slideshow {
        ...
        initEvents() {
            ...
            //         item.querySelector('.back').addEventListener('click', () => this.closeSlide());
            //     });
            // }
            this.DOM.contentItems.forEach(item => {
                const backElement = item.querySelector('.img-wrap--content');

                // Add another class (e.g., "new-class") to the "back" element
                backElement.classList.add('.img-wrap--content');

                backElement.addEventListener('click', () => this.closeSlide());
            });
        }
        navigate(direction) {
            if (this.isAnimating) {
